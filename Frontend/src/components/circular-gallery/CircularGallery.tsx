  ...
  Object.getOwnPropertyNames(proto).forEach((key) => {
    if (key !== "constructor" && typeof (instance as Record<string, unknown>)[key] === "function") {
      (instance as Record<string, unknown>)[key] = (
        (instance as Record<string, () => void>)[key] as () => void
      ).bind(instance);
    }
  });
}
...
class App {
  ...
  medias: Media[] = [];
  destroyed = false;
  onCheckDebounce!: ReturnType<typeof debounce<typeof App.prototype.onCheck>>;
  screen!: ScreenSize;
  viewport!: ViewportSize;
  isDown = false;
  start = 0;
  ...
  onCheck() {
    if (this.destroyed || !this.medias?.length || !this.medias[0]) return;
